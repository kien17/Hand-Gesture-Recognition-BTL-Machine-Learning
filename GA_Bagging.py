...
def random_individual():
    return {
        ...
        "max_samples": random.uniform(0.5, 1.0)  # bootstrap ratio
    }
...
def fitness(individual, X_train, y_train, X_val, y_val):
    ...
    model = BaggingClassifier(
        ...
        max_samples=individual["max_samples"],
        bootstrap=True,
        n_jobs=-1,                 # Bagging ăn điểm ở đây
        random_state=42
    )
...
def mutate(individual, mutation_rate=0.2):
    ...
    if random.random() < mutation_rate:
        individual["max_samples"] = random.uniform(0.5, 1.0)
    return individual
...
import joblib
joblib.dump(final_model, "bagging_GA_best.pkl")
print("Final GA-Bagging model saved!")
